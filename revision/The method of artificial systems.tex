...
\maketitle
\begin{abstract}
Artificial life is embodied AI. By applying a small set of architectural
rules, including tight coupling between hardware and software, compact
behavior representations, asynchronous local process, in-node memory
words, explicit communication, and reproducible board-level experiment,
synthetic systems can be built and studied as situated, adaptive forms
of machine life. In this revision, that claim is grounded in the Volatco
board experiments as a repeatable environment for behavior, coordination,
energy, and adaptation under ordinary scientific conditions.
\end{abstract}
...
In this manuscript, that path is grounded in the Volatco board experiments,
where reset conditions, pin maps, watchdog behavior, message handoff,
power draw, and pass/fail criteria can be made explicit and repeated.
...
The supporting path is distributed across the companion papers. The
symbolic-logic study recovers the architectural unity of sensing,
state, control, action, and adaptation. The AIBO R-CODE study extracts
a compact behavioral vocabulary from preserved robot software. The
hybrid-substrate study carries that vocabulary toward execution through
node-local process, asynchronous communication, boot-time initialization,
and functional simulation. The present manuscript gathers those threads
into a single forward method and grounds them in the Volatco board
experiments.
