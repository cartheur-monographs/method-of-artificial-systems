...
forms of machine life. Read together with the companion studies on
symbolic logic and AIBO R-CODE, it advances a stronger claim: artificial
life can be treated as embodied AI whose behavior is organized through
asynchronous process and stabilized through in-node memory words. In
its present experimental form, that claim is centered on a reproducible
board-level research path: the Volatco asynchronous computing platform
as a test environment for local memory, explicit communication, power-aware
control, and repeatable embodied experiments.
\end{abstract}
...
The proof burden of this manifesto does not stand alone. It is distributed
across the sibling papers. The symbolic-logic study recovers the older
architectural claim that sensing, state, control, action, and adaptation
belong to one machine-intelligence design problem. The AIBO R-CODE
study extracts a compact behavioral vocabulary from preserved robot
software. The hybrid-substrate study then carries that vocabulary toward
execution by showing how node-local processes, asynchronous communication,
boot-time initialization, and functional simulation can serve as a
concrete embodiment path. In the current revision, that path should
be understood through the Volatco board experiments: not as a product
claim, but as a board-level scientific setting in which reset conditions,
pin maps, watchdog behavior, message handoff, power draw, and pass/fail
criteria can be made explicit and repeated.
...
Stated more directly in light of the companion papers, the program
proposed here treats artificial life as a form of embodied AI. Its
defining architecture is behavioral rather than merely representational:
asynchronous process organizes activity across the system, while in-node
memory words preserve local state, continuity, and adaptation. The
hybrid-substrate work matters here because it supplies the first serious
implementation proof of that formulation: the claim is not only conceptual,
but architectural and executable. The strongest current output of that
program is not a generalized robot mythology but a reproducible board
experiment in which these claims can be tested under ordinary scientific
conditions.
...
can be decomposed into asynchronous stateful processes. Taken together,
they support the present claim that artificial life should be engineered
as embodied AI with local memory-bearing nodes rather than as a centralized
disembodied intelligence. The hybrid-substrate study adds the missing
proof layer by showing a continuous path from behavior description to
distributed executable realization: node allocation, per-node images,
boot-descriptor initialization, functional simulation, and eventual
hardware manifestation on an asynchronous many-node substrate.
...
of the work, the strongest empirical center is the Volatco board experiment:
...
an engineering basis for interval-driven power use \cite{key-13}(438-43).
A pulse period
...
At present, that evaluation is most compelling when anchored to the
Volatco board as a reproducible substrate rather than to speculative
future robot generalities.
...
research sequence, however, the more decisive experimental template
is the Volatco board environment, because it combines asynchronous
node execution with explicit development and measurement hooks.
...
For this class of experiments, the setup should be understood not only
as a legacy measurement template for the Model 'F' transmission broadcast,
but as a bridge toward the present board-level substrate experiments.
The methodological requirements remain the same:
\begin{itemize}
...
The same scientific discipline now applies to the Volatco board. Reset
mode, boot mode, watchdog state, serial programming path, pin assignment,
...
the current state of the work, the second should be centered on the
...
Volatco board serves as a concrete asynchronous substrate for studying
...
therefore comes from order, disorder, and entropy. This acknowledges
Erwin Schr\"odinger's priority in framing life through the maintenance
of order against equilibrium and the practical significance of negative
entropy \cite{key-22}. The
...
\item contemporary artificial-life techniques that privilege adaptation,
      with explicit acknowledgement of Steve Grand's priority in treating
      lifelike behavior as something grown through situated adaptive
      process rather than scripted display \cite{key-25},
...
\item philosophy only where it sharpens an engineering question, with
      acknowledgement of Jean Baudrillard's priority in forcing questions
      about simulation, representation, and the limits of appearance
      without treating that work here as a program for design \cite{key-26},
...
into stable embodied behavior. This acknowledges H. H. Pattee's priority
in arguing that artificial life requires a real epistemology linking
description, symbol, and embodied construction \cite{key-28}. 
...
\subsection{Intellectual Acknowledgements of Priority}

The older lineage behind this manifesto deserves explicit acknowledgement
without being turned into a review literature. Grey Walter established
...
artificial agency in view \cite{key-3,key-8,key-18}. Holland and McFarland
also deserve acknowledgement for giving artificial ethology a clear
programmatic form, while Rodney Brooks helped keep attention on the
problem of building materially embodied, behavior-producing systems
across the boundary between artificial intelligence and artificial
life \cite{key-11,key-2,key-24}. Ross Ashby remains central
for priority on regulation, stability under disturbance, and adaptive
organization as a state problem \cite{key-12,key-19}.
...
problem rather than a neutral condition of existence \cite{key-9,key-10,key-21}. Isaac Asimov
...
with computable description; and John
von Neumann for priority on self-reproduction as a design problem rather
than a mere metaphor \cite{key-14,key-20,key-27,key-23}. Andre Kurs,
Aristeidis Karalis, Robert Moffatt, J. D. Joannopoulos, Peter Fisher,
and Marin Solja\v{c}i\'{c} are acknowledged for giving wireless power
transfer a modern experimental footing, while Steve Grand's published
work is acknowledged for keeping open the possibility that artificial
life must be grown through situated adaptive process rather than assembled
as static demonstration; the private correspondence is retained here
as part of the project's own historical record rather than as public
literature \cite{key-16,key-15,key-25}.
